...
if (!is_file($configPath)) {
  ...
  echo json_encode(['error'=>'server_error','message'=>'Arquivo de configuração não encontrado: '.$configPath], JSON_UNESCAPED_UNICODE);
  exit;
}
...
try {
  ...
  if (!$email || !filter_var($email, FILTER_VALIDATE_EMAIL)) fail('E-mail inválido', 422);

  $nome     = trim((string)($in['nome']  ?? ''));
  $id_cargo = isset($in['id_cargo']) ? (int)$in['id_cargo'] : null; // <<< agora preferimos id_cargo (FK)
  $cargoTxt = trim((string)($in['cargo'] ?? ''));                   // compat: antigo fluxo por nome
  ...
  // Compatibilidade: se não vier id_cargo, tenta resolver por nome
  if (!$id_cargo && $cargoTxt !== '') {
    $st = $pdo->prepare("SELECT id_cargo FROM lp001_dom_cargos WHERE nome = ? LIMIT 1");
    $st->execute([$cargoTxt]);
    $id_cargo = (int)$st->fetchColumn();
    if (!$id_cargo) {
      $pdo->prepare("INSERT INTO lp001_dom_cargos (nome) VALUES (?)")->execute([$cargoTxt]);
    ...
    $sql = "
      UPDATE lp001_quiz_leads
         SET nome = COALESCE(NULLIF(:n_up, ''), nome),
             ...
             dt_consent = CASE WHEN :ct_up_dt = 1 OR :cm_up_dt = 1 THEN NOW() ELSE dt_consent END,
  ...
  http_response_code(500);
  echo json_encode(['error'=>'lead_start_failed','message'=>$e->getMessage()], JSON_UNESCAPED_UNICODE);
  exit;
}
