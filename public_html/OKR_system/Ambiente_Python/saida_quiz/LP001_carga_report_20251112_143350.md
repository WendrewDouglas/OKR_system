...
- **Data/Hora**: 2025-11-12 14:33:50
- **Versão alvo**: v1 (id_versao=9)
- **Arquivo**: LP001_carga_report_20251112_143350.md
...
SELECT COUNT(*) AS dominios FROM lp001_quiz_dominios WHERE id_versao={id_versao};
SELECT COUNT(*) AS perguntas FROM lp001_quiz_perguntas WHERE id_versao={id_versao};
SELECT COUNT(*) AS opcoes FROM lp001_quiz_opcoes WHERE id_pergunta IN
  (SELECT id_pergunta FROM lp001_quiz_perguntas WHERE id_versao={id_versao});

-- Overrides e vínculo cargo→versão
SELECT COUNT(*) AS overrides FROM lp001_quiz_domain_weights_overrides WHERE id_versao={id_versao};
SELECT c.nome AS cargo, m.id_versao
FROM lp001_quiz_cargo_map m
JOIN lp001_dom_cargos c ON c.id_cargo=m.id_cargo
WHERE m.id_versao={id_versao}
ORDER BY cargo;
```
...
[PATH] BASE=C:\Meus_Projetos\OKRsystem\public_html\OKR_System\Ambiente_python
       ...
       DOCS=C:\Meus_Projetos\OKRsystem\public_html\OKR_System\Ambiente_python\documents
...
[ PURGE ] Limpando conteúdo anterior do quiz (somente conteúdo; sem runtime)
  ...
  ✓ DELETE opcoes    | afetadas=124
  ✓ DELETE perguntas | afetadas=31
