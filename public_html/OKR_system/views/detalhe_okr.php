...
if (isset($_GET['ajax'])) {
  ...
  // require_cap de escrita removidos do topo global — devem ser verificados
  // dentro de cada ação específica que grava dados (apont_save, add_despesa, etc.)
  // Motivo: para ações de leitura (load_krs, kr_detail...) o id_kr/id_orcamento
  // vem null/0, fazendo o tenant-check falhar e retornar 403 para não-admin_master.
  ...
  /* ---------- EXCLUIR INICIATIVA (com cascata de orçamento/despesas/apontamentos) ---------- */
  if ($action === 'delete_iniciativa') {
    if (empty($_POST['csrf_token']) || $_POST['csrf_token'] !== ($_SESSION['csrf_token'] ?? '')) {
      http_response_code(403);
      echo json_encode(['success'=>false,'error'=>'Token CSRF inválido']); exit;
    }

    $id_ini = trim((string)($_POST['id_iniciativa'] ?? ''));
    if (!$id_ini) { echo json_encode(['success'=>false,'error'=>'ID da iniciativa ausente']); exit; }

    try {
      $pdo->beginTransaction();

      // 1) Despesas (orcamentos_detalhes) ligadas aos orçamentos da iniciativa
      if ($tableExists($pdo,'orcamentos_detalhes') && $tableExists($pdo,'orcamentos')) {
        $st = $pdo->prepare("
      ...
      // 4) A iniciativa em si
      $st = $pdo->prepare("DELETE FROM iniciativas WHERE id_iniciativa = :id");
      $st->execute(['id'=>$id_ini]);
      $deleted = $st->rowCount();
      ...
      if ($deleted) {
        ...
        echo json_encode(['success'=>false,'error'=>'Iniciativa não encontrada']);
      }
      exit;
    } catch (Throwable $e) {
      if ($pdo->inTransaction()) $pdo->rollBack();
      error_log('delete_iniciativa: '.$e->getMessage());
      echo json_encode(['success'=>false,'error'=>'Falha ao excluir iniciativa']);
      exit;
    }
  }
...
foreach ($milestones as $m) {
  ...
  if ($action === 'iniciativas_list') {
    ...
    // Busca em lote todos os envolvidos para as iniciativas deste KR
    $iniIds = array_column($iniciativas, 'id_iniciativa');
    $envolvMap = []; // id_iniciativa => [{id_user, nome}, ...]
    if ($iniIds) {
      $ph = implode(',', array_fill(0, count($iniIds), '?'));
      $stE = $pdo->prepare("
    ...
    foreach ($iniciativas as $ini) {
      ...
      $envolvidos = $envolvMap[$id_ini] ?? [];
      $respNome = $ini['responsavel_nome'] ?: '—';
      if ($envolvidos) {
        $respNome = format_envolvidos_nomes($envolvidos);
      }
    ...
    if ($action === 'nova_iniciativa') {
      ...
      // Se veio lista, usa o 1º como id_user_responsavel denormalizado
      if ($respIds) {
        $resp = $respIds[0];
      }
      ...
      try {
        ...
        $syncIds = $respIds ?: [$resp ?: (int)$_SESSION['user_id']];
...
<head>
  ...
  <style>
    ...
    .obj-actions{ display:flex; gap:10px; margin-top:12px; align-items:center; }
    .obj-actions .spacer{ flex:1; }
    ...
    /* Multi-responsavel picker */
    .resp-picker{ position:relative; }
    .resp-selected{ display:flex; flex-wrap:wrap; gap:4px; min-height:38px; padding:6px 8px; background:#0b0f14; border:1px solid var(--border); border-radius:10px; cursor:pointer; align-items:center; }
    .resp-selected:empty::after{ content:'Selecione responsáveis...'; color:#6b7280; font-size:.85rem; }
    .resp-chip{ display:inline-flex; align-items:center; gap:4px; background:#1f2937; border:1px solid #374151; color:#d1d5db; padding:3px 8px; border-radius:999px; font-size:.78rem; font-weight:600; white-space:nowrap; }
    .resp-chip .chip-x{ cursor:pointer; color:#9ca3af; font-size:.7rem; margin-left:2px; }
    .resp-chip .chip-x:hover{ color:#f87171; }
    .resp-dropdown{ display:none; position:absolute; top:100%; left:0; right:0; z-index:100; max-height:200px; overflow-y:auto; background:#0f1420; border:1px solid var(--border); border-radius:10px; margin-top:4px; padding:4px 0; }
    .resp-dropdown.open{ display:block; }
    .resp-dropdown label{ display:flex; align-items:center; gap:8px; padding:6px 12px; cursor:pointer; font-size:.85rem; color:#d1d5db; }
    .resp-dropdown label:hover{ background:#1f2937; }
    .resp-dropdown input[type=checkbox]{ width:16px; height:16px; accent-color:var(--gold); }
    .resp-pill{ display:inline-flex; align-items:center; gap:4px; background:#0b0f14; border:1px solid var(--border); color:#a6adbb; padding:3px 8px; border-radius:999px; font-size:.75rem; font-weight:600; margin:1px 2px; }
    /* Filtros */
    ...
    <main class="okr-detail">
      ...
      <?php
        $breadcrumbs = [
          ...
          ['label' => 'Detalhe do Objetivo', 'icon' => 'fa-solid fa-circle-info'],
        ];
      ...
      <section class="obj-card">
        ...
        <div class="obj-actions">
          ...
          <?php if (has_cap('W:objetivo@ORG')): ?>
            <span class="spacer"></span>
            <button class="btn btn-danger" id="btnDeleteObj"
                    data-id="<?= (int)$objetivo['id_objetivo'] ?>">
              <i class="fa-regular fa-trash-can"></i>&nbsp;Excluir Objetivo
            </button>
          <?php endif; ?>
        </div>
      ...
      <section class="kpi-grid">
        <div class="kpi">
          ...
          <div class="kpi-value" id="kpiTotalKrs" data-kpi="total_krs">0</div>
          <div class="kpi-sub">Críticos: <strong id="kpiCriticos" data-kpi="criticos">0</strong> · Em risco: <strong id="kpiRisco" data-kpi="em_risco">0</strong></div>
        </div>
        <div class="kpi">
    ...
    <div class="body">
      ...
      <form id="formNovaIniciativa">
        ...
        <div class="mb-2">
          ...
          <input type="hidden" name="id_user_responsavel" id="ni_resp_hidden">
          <input type="hidden" name="responsaveis_json" id="ni_resp_json">
          <div class="resp-picker" id="ni_resp_picker">
            <div class="resp-selected" id="ni_resp_chips"></div>
            <div class="resp-dropdown" id="ni_resp_dropdown"></div>
          </div>
        </div>
...
<aside id="drawerIniStatus" class="drawer" aria-hidden="true">
  ...
  <script>
    ...
    $('#btnSalvarIni')?.addEventListener('click', async ()=>{
      ...
      // Validação: pelo menos 1 responsável
      const respJson = $('#ni_resp_json')?.value || '[]';
      const respArr = JSON.parse(respJson);
      if (!respArr.length) { toast('Selecione ao menos 1 responsável.', false); return; }
    ...
    // ====== Multi-responsavel picker helpers ======
    function updateRespChips(){
      const chips = $('#ni_resp_chips');
      const dd    = $('#ni_resp_dropdown');
      const checks = dd.querySelectorAll('input[type=checkbox]:checked');
      const ids = [];
      chips.innerHTML = '';
      checks.forEach(chk => {
        const uid  = chk.value;
        const nome = chk.getAttribute('data-nome') || uid;
        ids.push(parseInt(uid, 10));
        const chip = document.createElement('span');
        chip.className = 'resp-chip';
        chip.innerHTML = `${escapeHtml(nome)} <span class="chip-x" data-uid="${uid}">&times;</span>`;
        chips.appendChild(chip);
      });
      // Hidden inputs
      $('#ni_resp_json').value   = JSON.stringify(ids);
      $('#ni_resp_hidden').value = ids[0] || '';
    }

    // Toggle dropdown on click
    $('#ni_resp_chips')?.addEventListener('click', (e) => {
      if (e.target.closest('.chip-x')) {
        const uid = e.target.closest('.chip-x').getAttribute('data-uid');
        const chk = $('#ni_resp_dropdown').querySelector(`input[value="${uid}"]`);
        if (chk) { chk.checked = false; updateRespChips(); }
        return;
      }
      $('#ni_resp_dropdown').classList.toggle('open');
    });

    // Close dropdown on outside click
    document.addEventListener('click', (e) => {
      if (!e.target.closest('#ni_resp_picker')) {
        $('#ni_resp_dropdown')?.classList.remove('open');
      }
    });
    ...
    async function openNovaIniciativaDrawer(id){
      ...
      $('#ni_resp_chips').innerHTML = '';
      $('#ni_resp_json').value = '[]';
      $('#ni_resp_hidden').value = '';
      $('#ni_resp_dropdown').classList.remove('open');
      $('#ni_resp_dropdown').querySelectorAll('input[type=checkbox]').forEach(c => c.checked = false);
      previsoes = [];
      renderPrev();
      $('#ni_id_kr').value = id;
      ...
      // 2) responsáveis (mesma company) — multi-select com checkboxes
      try{
        const res  = await fetch(`${SCRIPT}?ajax=list_responsaveis_company`);
        const data = await res.json();
        const dd   = $('#ni_resp_dropdown');
        dd.innerHTML = '';
        window._niRespUsers = data.items || [];
        window._niRespUsers.forEach(u=>{
          const ln  = (u.ultimo_nome || '').trim();
          const nome = ln ? `${u.primeiro_nome} ${ln}` : (u.primeiro_nome || u.id_user);
          const lbl = document.createElement('label');
          lbl.innerHTML = `<input type="checkbox" value="${u.id_user}" data-nome="${escapeHtml(nome)}"> ${escapeHtml(nome)}`;
          lbl.querySelector('input').addEventListener('change', updateRespChips);
          dd.appendChild(lbl);
        });
        // Pré-seleciona o usuário logado
        const chk = dd.querySelector(`input[value="${currentUserId}"]`);
        if (chk) { chk.checked = true; updateRespChips(); }
        else { updateRespChips(); }
      } catch(e){
        $('#ni_resp_dropdown').innerHTML = '<label style="color:#f87171">Falha ao carregar</label>';
      }
    ...
    async function loadKRs(){
      ...
      try {
        ...
        console.error('loadKRs fetch error:', e);
        cont.innerHTML = `<div class="chip" style="background:#5b1b1b;color:#ffe4e6;border-color:#7a1020"><i class="fa-solid fa-triangle-exclamation"></i> Erro ao carregar KRs</div>`;
        return;
      }

      if(!data.success){
        cont.innerHTML = `<div class="chip" style="background:#5b1b1b;color:#ffe4e6;border-color:#7a1020"><i class="fa-solid fa-triangle-exclamation"></i> ${escapeHtml(data.message || data.error || 'Erro ao carregar')}</div>`;
        return;
      }

      // Sem KRs cadastrados — mostra estado vazio com ação de criar
      if (!data.krs || data.krs.length === 0) {
        cont.innerHTML = `
          <div style="text-align:center;padding:32px 16px;color:#9aa4b2;">
            <i class="fa-solid fa-folder-open" style="font-size:2rem;margin-bottom:12px;display:block;opacity:.5"></i>
            <p style="margin:0 0 12px;font-weight:600;color:#e6e9f2;">Nenhum Key Result cadastrado para este objetivo</p>
            <a href="/OKR_system/views/novo_key_result.php?id_objetivo=${idObjetivo}" class="btn btn-gold btn-sm">
              <i class="fa-solid fa-plus"></i> Cadastrar Key Result
            </a>
          </div>`;
        return;
      }
    ...
    document.addEventListener('click', async (e)=>{
      ...
      // Excluir iniciativa
      const btnIniDel = e.target.closest('button[data-act="ini-delete"]');
      if (btnIniDel) {
        const idIni = btnIniDel.getAttribute('data-id');
        const desc  = btnIniDel.getAttribute('data-desc') || '';
        const msg   = `Tem certeza que deseja excluir a iniciativa?\n\n"${desc}"\n\nIsso removerá também orçamentos e despesas vinculados. Esta ação é irreversível.`;
        ...
        try {
          ...
          const res = await fetch(`${SCRIPT}?ajax=delete_iniciativa`, { method:'POST', body: fd });
          const data = await res.json();
          if (!data.success) { toast(data.error || 'Falha ao excluir iniciativa', false); return; }

          toast('Iniciativa excluída com sucesso.');
          const open = document.querySelector('.kr-card.open');
          if (open) await loadIniciativas(open.getAttribute('data-id'));
        } catch(err) {
          toast('Erro ao excluir iniciativa: ' + (err.message||err), false);
        }
        return;
      }
    ...
    // Excluir Objetivo (permanente)
    const btnDeleteObj = document.getElementById('btnDeleteObj');
    if (btnDeleteObj) {
      btnDeleteObj.addEventListener('click', async () => {
        const id = btnDeleteObj.getAttribute('data-id');
        const msg = '⚠️ ATENÇÃO!\n\n'
          + 'Excluir este objetivo é PERMANENTE e removerá:\n'
          + '- Todos os Key Results vinculados\n'
          + '- Milestones, apontamentos e comentários\n'
          + '- Iniciativas e orçamentos\n\n'
          + 'Esta ação NÃO poderá ser desfeita.\n\nDeseja EXCLUIR mesmo assim?';
        if (!confirm(msg)) return;

        const fd = new FormData();
        fd.append('csrf_token', csrfToken);
        fd.append('id_objetivo', id);

        try {
          const res = await fetch('/OKR_system/auth/delete_objetivo.php', {
            method: 'POST', body: fd
          });
          const data = await res.json();
          if (!data.success) {
            toast(data.error || 'Falha ao excluir objetivo', false);
            return;
          }
          toast('Objetivo excluído definitivamente.');
          setTimeout(() => { window.location.href = '/OKR_system/views/meus_okrs.php'; }, 1200);
        } catch (e) {
          toast('Erro de rede ao excluir objetivo', false);
        }
      });
    }
    ...
    async function loadIniciativas(id){
      ...
      // Ícone de saquinho de dinheiro (lançar despesa) se houver orçamento
        ...
        // Ícone para excluir iniciativa
        const deleteBtn = `<button class="btn btn-outline btn-sm" title="Excluir iniciativa"
                            data-act="ini-delete" data-id="${ini.id_iniciativa}"
                            data-desc="${escapeHtml(ini.descricao||'')}"
                            style="color:#f87171">
        ...
        // Monta pills de responsáveis
        let respHtml = '';
        if (ini.responsaveis && ini.responsaveis.length) {
          respHtml = ini.responsaveis.map(r =>
            `<span class="resp-pill">${escapeHtml(r.nome.split(' ')[0])}</span>`
          ).join('');
        } else {
          respHtml = escapeHtml(ini.responsavel||'—');
        }
    ...
    // (Duplicate listeners for btnSalvarIni / btnSalvarDesp removed —
    //  primary handlers are registered above, near line 3008 / 3049)
