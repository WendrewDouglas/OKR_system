...
if ($respIds) {
  $id_resp = (string)$respIds[0]; // 1º será o denormalizado
}
...
try {
  ...
  // Valida múltiplos responsáveis (se enviados)
  if (count($respIds) > 1 && $userCompany !== null) {
    $ph = implode(',', array_fill(0, count($respIds), '?'));
    $st = $pdo->prepare("SELECT id_user, id_company FROM usuarios WHERE id_user IN ($ph)");
    $st->execute($respIds);
    $rows = $st->fetchAll();
    if (count($rows) !== count($respIds)) {
      throw new RuntimeException('Um ou mais responsáveis informados não existem.');
    }
    foreach ($rows as $r) {
      if ($r['id_company'] !== null && (string)$r['id_company'] !== (string)$userCompany) {
        throw new RuntimeException('Todos os responsáveis devem pertencer à mesma organização.');
      }
    }
  }
  ...
  // Notifica aprovadores sobre novo orçamento pendente (somente se incluiu orçamento)
  if ($incl_orc) {
      try {
          ...
          if ($creatorRow && !empty($creatorRow['id_company'])) {
              notify_approvers_new_item($pdo, 'orcamento', $id_ini, [
                  ...
                  'extras'       => [
                      ...
                      'valor'         => $valor_orc,
                      'justificativa' => $just_orc,
                  ],
              ]);
          }
      } catch (Throwable $e) {
          error_log('salvar_iniciativas notify: ' . $e->getMessage());
      }
  }
