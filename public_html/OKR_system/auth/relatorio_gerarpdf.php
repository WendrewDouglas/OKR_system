...
try {
  ...
  error_log('relatorio_gerarpdf conexão: '.$e->getMessage());
  http_response_code(500); die('Falha ao processar. Tente novamente ou contate o administrador.');
}
...
function kpis(PDO $pdo, int $id): array {
  $st1=$pdo->prepare("SELECT COUNT(*) FROM iniciativas i INNER JOIN key_results kr ON kr.id_kr=i.id_kr WHERE kr.id_objetivo=?");
  $st1->execute([$id]); $tIni=(int)($st1->fetchColumn() ?: 0);
  $st2=$pdo->prepare("SELECT COUNT(DISTINCT i.id_iniciativa) FROM iniciativas i INNER JOIN key_results kr ON kr.id_kr=i.id_kr INNER JOIN orcamentos o ON o.id_iniciativa=i.id_iniciativa WHERE kr.id_objetivo=?");
  $st2->execute([$id]); $comOrc=(int)($st2->fetchColumn() ?: 0);
  $st3=$pdo->prepare("SELECT COALESCE(SUM(o.valor),0) FROM orcamentos o INNER JOIN iniciativas i ON i.id_iniciativa=o.id_iniciativa INNER JOIN key_results kr ON kr.id_kr=i.id_kr WHERE kr.id_objetivo=?");
  $st3->execute([$id]); $aprov=(float)($st3->fetchColumn() ?: 0);
  $st4=$pdo->prepare("SELECT COALESCE(SUM(od.valor),0) FROM orcamentos_detalhes od INNER JOIN orcamentos o ON o.id_orcamento=od.id_orcamento INNER JOIN iniciativas i ON i.id_iniciativa=o.id_iniciativa INNER JOIN key_results kr ON kr.id_kr=i.id_kr WHERE kr.id_objetivo=?");
  $st4->execute([$id]); $real=(float)($st4->fetchColumn() ?: 0);
