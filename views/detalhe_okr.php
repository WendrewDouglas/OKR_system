...
if ($action === 'list_responsaveis_company') {
  try {
    ...
    // tenta achar a coluna id_company
    $cols = $pdo->query("SHOW COLUMNS FROM `usuarios`")->fetchAll(PDO::FETCH_ASSOC);
    $names = array_column($cols,'Field');
    $companyCol = in_array('id_company',$names,true) ? 'id_company' : null;

    // company do usuário logado (se existir a coluna)
    $companyVal = null;
    if ($companyCol) {
      $st = $pdo->prepare("SELECT `$companyCol` FROM `usuarios` WHERE id_user=:u LIMIT 1");
      $st->execute(['u'=>$userId]);
      $companyVal = $st->fetchColumn();
    }

    if ($companyCol && $companyVal !== null && $companyVal !== '') {
      $st = $pdo->prepare("
    ...
    echo json_encode(['success'=>false,'error'=>'Falha ao listar responsáveis']); exit;
  }
}
...
if (isset($_GET['ajax'])) {
  ...
  // Adiciona comentário ao KR (melhor esforço, tentando tabelas conhecidas; senão fallback em campo texto do KR)
  $addKrComment = static function(PDO $pdo, string $id_kr, int $id_user, string $texto): void {
    ...
    foreach (['kr_comentarios','comentarios_kr'] as $t) {
      try {
        $pdo->query("SHOW COLUMNS FROM `$t`");
      } catch (Throwable $e) { continue; }

      // Mapeia nomes de colunas possíveis
      $idKrCol = null;
    ...
    foreach (['observacoes','comentarios'] as $c) {
      try {
        $col = $pdo->query("SHOW COLUMNS FROM `key_results` LIKE '$c'")->fetch();
        if (!$col) continue;
      } catch (Throwable $e) { continue; }
  ...
  // Descobre o nome de coluna que referencia o KR em uma tabela (id_kr, kr_id, etc.)
  ...
  if ($action === 'load_krs') {
    ...
    try {
      ...
      echo json_encode(['success'=>false,'error'=>'Falha ao consultar KRs']); exit;
    }
  ...
  if ($action === 'kr_detail') {
    ...
    $st = $pdo->prepare("
      SELECT id_kr, id_objetivo, key_result_num, descricao, farol, status,
            ...
            tipo_frequencia_milestone" . ($hasUlt ? ", dt_ultima_atualizacao" : ", NULL AS dt_ultima_atualizacao") . "
      FROM key_results
      WHERE id_kr = :id
      LIMIT 1
    ");
    ...
    // Detecta nome da tabela de milestones e colunas disponíveis
    $msTable = null;
    ...
    // Preferências de nomes por coluna
    ...
    if (!$dateCol || !$expCol) {
      echo json_encode(['success'=>false,'error'=>'Colunas de milestones não encontradas (data/esperado)']); exit;
    }

    // Monta SELECT dinamicamente conforme colunas existentes
...
if ($action === 'orc_dashboard') {
  ...
  try {
    ...
    // Totais gerais (sem filtro de ano)
    $sqlA = "SELECT COALESCE(SUM(o.valor),0) 
             FROM orcamentos o 
             INNER JOIN iniciativas i ON i.id_iniciativa=o.id_iniciativa
             WHERE i.id_kr=:id";
    $sqlR = "SELECT COALESCE(SUM(od.valor),0)
             FROM orcamentos_detalhes od
             INNER JOIN orcamentos o ON o.id_orcamento=od.id_orcamento
             INNER JOIN iniciativas i ON i.id_iniciativa=o.id_iniciativa
             WHERE i.id_kr=:id";
    ...
    // Planejado/Realizado até hoje (filtrando pela data <= hoje)
    $sa2 = $pdo->prepare($sqlA . " AND o.data_desembolso <= CURDATE()");
    $sa2->execute(['id'=>$id_kr]); $planAtHoje = (float)$sa2->fetchColumn();
    $sr2 = $pdo->prepare($sqlR . " AND od.data_pagamento <= CURDATE()");
    $sr2->execute(['id'=>$id_kr]); $realAtHoje = (float)$sr2->fetchColumn();
    $desvio = $realAtHoje - $planAtHoje;
    $farol = (abs($aprovado) < 0.01) ? 'neutro'
    ...
    $byPlan = []; foreach ($stmtPlan as $r) $byPlan[$r['comp']] = (float)$r['val'];
    ...
    $byReal = []; foreach ($stmtReal as $r) $byReal[$r['comp']] = (float)$r['val'];

    $stmtPend = $pdo->prepare("
      SELECT DATE_FORMAT(o.data_desembolso,'%Y-%m') AS comp, COUNT(*) AS qtd
      FROM orcamentos o
      INNER JOIN iniciativas i ON i.id_iniciativa=o.id_iniciativa
      WHERE i.id_kr=:id AND YEAR(o.data_desembolso)=:ano AND COALESCE(o.status_aprovacao,'pendente')='pendente'
      GROUP BY comp
    "); $stmtPend->execute($bind);
    $byPend = []; foreach ($stmtPend as $r) $byPend[$r['comp']] = (int)$r['qtd'];

    $series = [];
    $planAc=0; $realAc=0;
    for($m=1;$m<=12;$m++){
      $comp = sprintf('%04d-%02d',$ano,$m);
      $p = $byPlan[$comp] ?? 0.0; $r = $byReal[$comp] ?? 0.0;
      $planAc += $p; $realAc += $r;
      $series[] = [
        'competencia' => $comp,
        'planejado'   => round($p,2),
    ...
    $sqlInis = "
      ...
      ORDER BY i.num_iniciativa ASC, i.dt_criacao ASC
    ";
    $stInis = $pdo->prepare($sqlInis); $stInis->execute(['id'=>$id_kr]);
    $iniItems = [];
    ...
    echo json_encode(['success'=>false,'error'=>'Falha ao carregar dashboard de orçamentos']); exit;
  }
}
...
if ($action === 'list_status_kr') {
  try {
    ...
    echo json_encode(['success'=>false,'error'=>'Tabela dom_status_kr não encontrada']); exit;
  }
  $names = array_column($cols,'Field');
  ...
  // opcional: filtrar "cancelado" quando for reativar
  $onlyActive = isset($_GET['only_active']) ? (int)$_GET['only_active'] : 0;
  if ($onlyActive) {
    $norm = fn($s)=>mb_strtolower(trim((string)$s));
    $rows = array_values(array_filter($rows, fn($r)=> strpos($norm($r['id']), 'cancel')===false && strpos($norm($r['label']), 'cancel')===false ));
  }
...
if ($action === 'reactivate_kr') {
  ...
  $id_kr = $_POST['id_kr'] ?? '';
  $statusTarget = trim($_POST['status_target'] ?? ''); // << novo: status desejado
  if (!$id_kr){ echo json_encode(['success'=>false,'error'=>'id_kr inválido']); exit; }

  try {
    $pdo->beginTransaction();

    // 1) coluna de status em key_results
    $statusCol = null;
    ...
    // 2) detectar colunas da dom_status_kr (id + rótulo)
    $cols = $pdo->query("SHOW COLUMNS FROM `dom_status_kr`")->fetchAll(PDO::FETCH_ASSOC);
    ...
    $norm = function($s){
      ...
      $s2 = @iconv('UTF-8','ASCII//TRANSLIT',$s); // remove acentos
      if ($s2 !== false && $s2 !== null) $s = $s2;
      $s = preg_replace('/\s+/',' ',$s);
      return $s;
    };

    // 3) resolver o ID alvo
    $targetId = null;
    if ($statusTarget !== '') {
      ...
      // preferências quando nada vier do front
      ...
      if ($targetId === null) {
        // primeiro que não seja "cancelado"
        foreach ($rows as $r) {
    ...
    // 4) atualizar (bind como STRING, pois id_status é textual)
    ...
    echo json_encode(['success'=>false,'error'=>'Falha ao reativar KR: '.$e->getMessage()]); exit;
  }
}
  ...
  if ($action === 'cancel_kr') {
    ...
    try {
      ...
      // coluna de status em key_results
      $statusCol = null;
      foreach (['status','situacao','state'] as $c) {
        $st = $pdo->prepare("SHOW COLUMNS FROM `key_results` LIKE :c");
        $st->execute(['c'=>$c]);
      ...
      // Tabela dom_status_kr (id/status textual + rótulo)
      $cancelId = null;
      try {
        $cols = $pdo->query("SHOW COLUMNS FROM `dom_status_kr`")->fetchAll(PDO::FETCH_ASSOC);
        if ($cols) {
          $names = array_column($cols,'Field');
          $idCol = in_array('id_status',$names,true) ? 'id_status'
                : (in_array('id',$names,true) ? 'id' : $names[0]);
          $labelCol = in_array('descricao_exibicao',$names,true) ? 'descricao_exibicao'
                    : (in_array('descricao',$names,true) ? 'descricao'
                    : (in_array('nome',$names,true) ? 'nome' : ($names[1] ?? $names[0])));
          ...
          // 1) tenta match exato "cancelado"
          foreach ($rows as $r) {
            if ($norm($r['id'])==='cancelado' || $norm($r['label'])==='cancelado') { $cancelId = (string)$r['id']; break; }
          }
          // 2) qualquer item que contenha "cancel"
          if ($cancelId === null) {
            foreach ($rows as $r) {
              if (strpos($norm($r['id']), 'cancel')!==false || strpos($norm($r['label']), 'cancel')!==false) { $cancelId = (string)$r['id']; break; }
            }
          }
        }
      } catch (Throwable $e) {
        // ignora: trataremos abaixo
      }

      if ($cancelId === null) {
        // Se NÃO houver dom_status_kr, grava texto livre (sem FK)
        // Se houver FK e ainda assim cair aqui, preferimos falhar com msg clara.
        $hasDom = isset($rows);
        if ($hasDom) { throw new RuntimeException('Status "cancelado" não encontrado em dom_status_kr'); }
        $cancelId = 'Cancelado';
      }

      // Atualiza como STRING (sem PARAM_INT!)
      $st = $pdo->prepare("UPDATE `key_results` SET `{$statusCol}` = :v WHERE id_kr = :id");
      ...
      echo json_encode(['success'=>false,'error'=>'Falha ao cancelar KR: '.$e->getMessage()]); exit;
    }
  }
  ...
  if ($action === 'delete_kr') {
    ...
    try {
      ...
      // pega o id_objetivo do KR que será excluído
      ...
      // 2) Apaga milestones (em qualquer das tabelas conhecidas)
      foreach (['milestones_kr','milestones'] as $t) {
      ...
      // 3) Apaga apontamentos (se existirem)
      ...
      // 5) Finalmente, apaga o KR
      $st = $pdo->prepare("DELETE FROM key_results WHERE id_kr=:id");
      $st->execute(['id'=>$id_kr]);

      // 6) RENNUMERA os KRs restantes do objetivo (1..N), mantendo a ordem atual
      ...
      $pdo->rollBack();
      echo json_encode(['success'=>false,'error'=>'Falha ao excluir KR']); exit;
    }
  }

  // Fallback para ação não reconhecida
...
<head>
  ...
  <style>
    ...
    /* KRs */
    /* Escopar o flex apenas ao bloco da direita do detalhe (não aos KPIs principais) */
    .tabpane .kpi { display:flex; flex-direction:column; }
    .kr-ops-inline{
      margin-top:auto;
      display:flex; gap:8px; flex-wrap:wrap;
      border-top:1px dashed #273043; padding-top:10px;
    }
    ...
    .kr-ops-inline .btn{
      ...
      display: inline-flex;  /* alinha ícone + texto */
      align-items: center;
    ...
    /* (Opcional) ligeiro ajuste em telas muito pequenas */
    @media (max-width: 480px){
      .kr-ops-inline .btn{ width: 112px; }
    }

    /* Coluna direita com empilhamento vertical (card em cima, botões fora abaixo) */
    .kr-resumo-right{
      display: flex;
      flex-direction: column;
    }

    /* Barra de botões fora do card */
    .kr-ops-outside{
      margin-top: 8px;
      display: flex;
      gap: 8px;
    ...
    /* Tamanho fixo/menor dos botões (padrão) — só nesta barra */
    .kr-ops-outside .btn{
      padding: 6px 10px;
      font-size: .60rem;
      height: 34px;
    ...
    @media (max-width: 480px){
      .kr-ops-outside .btn{ width: 112px; }
    }
    ...
    .meta-pill.white,
    .meta-pill.white i { color:#eee !important; }
    ...
    /* título do KR cancelado com cinza médio */
    .kr-card.cancelado .kr-title { color:#9aa4b2; }
    /* Esconde ações quando o KR estiver cancelado */
    .kr-card.cancelado [data-act="apont"],
    .kr-card.cancelado [data-act="nova"]{
    ...
    /* Barra de ações do KR (na aba Resumo) */
    ...
    /* Banner com nome do KR no topo do drawer */
    .kr-banner {
...
<body>
  ...
  <aside id="drawerNovaIni" class="drawer" aria-hidden="true">
    ...
    <div class="body">
      ...
      <form id="formNovaIniciativa">
        ...
        <!-- será preenchido em JS com o JSON dos desembolsos -->
        ...
        <div class="mb-2">
          ...
          <!-- usa os mesmos status do dom_status_kr -->
  ...
  <script>
    ...
    try{
      ...
      const data = await res.json();
      const sel = $('#ni_resp'); sel.innerHTML = '';
      ...
      if (sel.options.length) {
        ...
        // se por algum motivo o logado não estiver na lista, fica no primeiro
    ...
    try{
      ...
      const data = await res.json();
      const sel = $('#ni_status'); sel.innerHTML = '';
      (data.items||[]).forEach(s=>{
        const opt = document.createElement('option');
        opt.value = s.id;                       // usa o id_status textual
        opt.textContent = s.label || s.id;
        sel.appendChild(opt);
      });
      // preferência: "nao iniciado" depois "em andamento"
...
function updatePrevTotals(){
  ...
  const ok = Math.abs(sum - total) < 0.005; // tolerância de 0,5 centavo
  $('#ni_prev_check').textContent = ok ? 'ok' : 'inconsistente';
  $('#ni_prev_check').className   = ok ? 'ni-ok' : 'ni-err';
  // atualizar hidden com JSON
...
$('#ni_prev_add')?.addEventListener('click', ()=>{
  ...
  const label = `${m}/${y}`;
  // regra: se já existir a competência, soma
...
$('#ni_valor_total')?.addEventListener('input', updatePrevTotals);
      ...
      data.krs.forEach(kr=>{
        ...
        cont.insertAdjacentHTML('beforeend', `
          <article class="kr-card${isCancel ? ' cancelado' : ''}" data-id="${id}">
            <div class="kr-head">
              <div>
                  <div class="kr-title"><i class="fa-solid fa-flag"></i>
                    KR${kr.key_result_num ? ' ' + kr.key_result_num : ''}: ${escapeHtml(truncate(kr.descricao||'', 160))}
                  </div>
                  <div class="meta-line">
      ...
      return `
        ...
        <div class="tabpane active" id="resumo-${id}">
          <div style="display:grid;grid-template-columns:2fr 1fr;gap:12px;align-items:stretch;">
            ...
            <!-- Direita: card de KPIs em cima + barra de botões fora do card -->
        ...
        <div class="tabpane" id="orc-${id}">
          <!-- Topbar: totais + filtro de ano + modo gráfico -->
          ...
          <div class="orc-card">
            ...
            <div class="table-wrap">
              <table class="table">
                <thead>
                  <tr>
                    <th>#</th><th>Iniciativa</th><th>Responsável</th>
          ...
          <!-- Base: pendências e últimas despesas -->
    ...
    // Delegação: toggle, tabs, botões (inclui ações de KR)
    document.addEventListener('click', async (e)=>{
      ...
      if (tabBtn){
        ...
        // se for a aba Orçamentos, carrega/atualiza
      ...
      if (btnCancel){
        ...
        const fd = new FormData(); fd.append('csrf_token', csrfToken); fd.append('id_kr', id); fd.append('justificativa', justificativa.trim());
      ...
      try{
        ...
        (data.items||[]).forEach(it=>{
          const opt = document.createElement('option');
          opt.value = it.id;                               // id_status (ex.: "em andamento")
          opt.textContent = it.label || it.id;             // descricao_exibicao (ex.: "Em Andamento")
          sel.appendChild(opt);
        });
    ...
    $('#btnReativarKrSave')?.addEventListener('click', async ()=>{
      ...
      if (btnExcluir){
        ...
        const fd = new FormData(); fd.append('csrf_token', csrfToken); fd.append('id_kr', id);
    ...
    async function loadKrDetail(id){
      ...
      // ---- Gráfico Curva "S" (apenas Esperado) ----
      ...
      if (ops){
        ...
        ops.innerHTML = `
          ...
          ${cancelado
            ? `<button class="btn btn-success" data-act="reactivate-kr" data-id="${id}"><i class="fa-solid fa-rotate-left"></i> Reativar KR</button>`
    ...
    function mesLabel(ym){ // 'YYYY-MM' -> 'MMM/YYYY'
    const [y,m] = (ym||'').split('-');
  ...
  async function loadOrcDashboard(id, ano){
    try{
      ...
      // Totais/topbar (reutiliza IDs antigos)
      ...
      // Chart (Mensal/Acumulado via segmented)
      ...
      if (seg && !seg.dataset.bound){
        ...
        seg.addEventListener('click', (e)=>{
          ...
          if (mode==='acum'){ orcCharts[id].config.type='line'; ds0.data = acumPlan; ds1.data = acumReal; }
          else { orcCharts[id].config.type='bar'; ds0.data = mensalPlan; ds1.data = mensalReal; }
          orcCharts[id].update();
        });
      }
      ...
      if (tb){
        ...
        if (!arr.length){ tb.innerHTML = `<tr><td colspan="6" class="empty">Sem iniciativas com orçamento.</td></tr>`; }
        else {
          arr.forEach(it=>{
            tb.insertAdjacentHTML('beforeend', `
              <tr>
      ...
      if (pend){
        ...
        if (!arr.length){ pend.innerHTML = `<div class="empty">Nenhuma parcela pendente.</div>`; }
        else {
          arr.forEach(p=>{
      ...
      if (ult){
        ...
        if (!arr.length){ ult.innerHTML = `<div class="empty">Nada lançado ainda.</div>`; }
        else {
          arr.forEach(d=>{
    ...
    $('#btnSalvarIni')?.addEventListener('click', async ()=>{
      ...
      // validações quando orçamento marcado
      ...
      if (incl){
        ...
        if (Math.abs(soma - total) > 0.005){
          toast('A soma das competências deve ser igual ao total.', false);
          return;
        }
      }
