...
if (isset($_GET['ajax'])) {
  ...
  if ($action === 'update' && $_SERVER['REQUEST_METHOD']==='POST') {
    try {
      ...
      // Período (opcional — depende do seu schema)
      ...
      $sql = "UPDATE objetivos
                 SET nome_objetivo         = :nome,
                     tipo                  = :tipo,
                     pilar_bsc             = :pilar,
                     responsavel           = :resp,     -- CSV
                     dono                  = :dono,     -- opcional
                     observacoes           = :obs,
                     qualidade             = :qualidade,
      ...
      $st->execute([
        ...
        ':pilar'=> $pilar,
        ':resp' => $responsavelCS,
        ':dono' => $dono,              // TODO: ajuste se não usar campo 'dono'
        ':obs'  => $obsUpd,
        ':qualidade' => $qualidade !== '' ? $qualidade : ($obj['qualidade'] ?? null),
        ':user' => $userId,
        ':id'   => $id,
        ':comp' => $companyId,
      ]);

      // Atualiza período se o seu schema possuir estas colunas
      if ($periodo_inicio && $periodo_fim) {
        // TODO: ajustar nomes de colunas se forem diferentes
        $sp = $pdo->prepare("UPDATE objetivos
                                SET periodo_inicio = :pi,
                                    periodo_fim    = :pf
                              WHERE id_objetivo=:id AND id_company=:c LIMIT 1");
        $sp->execute([':pi'=>$periodo_inicio, ':pf'=>$periodo_fim, ':id'=>$id, ':c'=>$companyId]);
      }
      ...
      http_response_code(500);
      echo json_encode(['success'=>false,'error'=>'Falha ao atualizar o objetivo.']);
      exit;
    }
  }
...
$respCSV       = (string)($OBJ['responsavel'] ?? ''); // CSV de ids
$obsExistente  = (string)($OBJ['observacoes'] ?? '');
$qualidade     = (string)($OBJ['qualidade'] ?? '');
$periodo_ini   = (string)($OBJ['periodo_inicio'] ?? '');
$periodo_fim   = (string)($OBJ['periodo_fim'] ?? '');

// Heurística: se tem período salvo, mostramos badge; defaulta ciclo "trimestral" para UI
$cicloDefault  = 'trimestral';
$periodBadge   = ($periodo_ini && $periodo_fim) ? ($periodo_ini.' → '.$periodo_fim) : '—';
...
<body>
    ...
    <main class="eobj">
      ...
      <section class="head-card">
        ...
        <div class="head-meta">
          ...
          <span id="periodBadge" class="pill pill-gold" style="<?= $periodo_ini && $periodo_fim ? '' : 'display:none;' ?>">
            <i class="fa-regular fa-calendar"></i>
            <span id="periodText"><?= h($periodBadge) ?></span>
          </span>
        </div>
      </section>
      ...
      <section class="form-card">
        ...
        <form id="editForm" action="?ajax=1&action=update" method="post" novalidate>
          ...
          <!-- Hidden para período calculado pelo JS -->
          ...
          <!-- Ciclo (UI idêntica ao novo_objetivo; usaremos cálculo de período via JS) -->
          <div class="grid-2 align-center" style="margin-top:12px;">
            ...
            <div id="ciclo_detalhe_wrapper" role="group" aria-labelledby="lblPeriodo">
              <label id="lblPeriodo"><i class="fa-regular fa-calendar"></i> Período <span class="helper">(se usar personalizado)</span></label>

              <!-- Mantemos as mesmas caixas do novo_objetivo para consistência -->
              ...
              <div id="ciclo_detalhe_personalizado" class="detalhe <?= ($periodo_ini && $periodo_fim)?'':'d-none' ?>">
                <div class="grid-2">
                  <input type="month" id="ciclo_pers_inicio" name="ciclo_pers_inicio" value="<?= $periodo_ini ? h(substr($periodo_ini,0,7)) : '' ?>">
                  <input type="month" id="ciclo_pers_fim"    name="ciclo_pers_fim"    value="<?= $periodo_fim ? h(substr($periodo_fim,0,7)) : '' ?>">
                </div>
              </div>
            </div>
          </div>
  ...
  <script>
    ...
    function updateBadges(){
      ...
      $('#periodo_inicio').value = startISO;
      $('#periodo_fim').value    = endISO;
    ...
    function toggleCycleDetail(){
      ...
      Object.entries(boxes).forEach(([k,el])=>{ if(!el) return; el.classList.toggle('d-none', k!==tipo && !(k==='personalizado' && tipo==='personalizado')); });
      updateBadges();
    }
    ...
    function setupOwners(){
      ...
      // Pré-popular chips a partir do hidden (valor inicial do PHP)
      function bootstrapChipsFromHidden(){
        const current = (hiddenResp.value || '').split(',').filter(Boolean);
      ...
      listCont.querySelectorAll('li').forEach(li => {
        li.addEventListener('click', () => {
          // Se já existe chip do mesmo usuário, ignora
    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      $('#btnSalvar')?.addEventListener('click', () => {
        // Validação básica antes de abrir justificativa
      ...
      $('#confirmJust')?.addEventListener('click', async () => {
        ...
        try {
          ...
          // Garante período calculado
          const {startISO, endISO} = computePeriodFromCycle();
          fd.set('periodo_inicio', startISO);
          fd.set('periodo_fim', endISO);
